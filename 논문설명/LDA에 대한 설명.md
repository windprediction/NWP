...
**예: 도시의 현재 기온 분포를 알고 싶다**

* 📱 **관측값**: 여러 지점의 실제 온도 측정값 (ex. 서울 30도, 부산 29도 등)
* 💻 **모델 배경장**: 수치예보 모델이 예측한 온도 분포
* 🧠 **3DVAR**: 이 둘을 잘 섞어서 **관측값을 따르되**, **모델이 말도 안 되는 값으로 튀는 걸 막는** 최적의 상태를 찾는 알고리즘!
...
1. \*\*목표함수(J)\*\*를 정의한다:
   ...
   * $x$: 우리가 찾는 ‘최적 상태’ (예: 현재의 온도분포)
   * $x_b$: 모델 배경장
   * $y$: 관측값
   * $H(x)$: 관측 위치로 모델 값을 변환해주는 함수
   * $B$, $R$: 모델 오류, 관측 오류의 공분산 행렬

2. 이 함수 $J(x)$를 **최소화**해서 가장 그럴듯한 상태 $x$를 찾는다.
...
> “**관측값에 너무 끌려가지도 않고, 모델 값만 맹신하지도 않으며**, 두 정보를 신뢰도에 따라 잘 섞어서 ‘가장 가능성 높은 대기 상태’를 찾아내는 방법!”

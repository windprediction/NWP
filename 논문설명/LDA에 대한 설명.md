...
---
3DVAR(Three-Dimensional Variational Data Assimilation)은 기상예보나 대기 시뮬레이션 등에서 **관측값과 모델 예측값을 잘 조합해서 가장 현실에 가까운 상태를 추정**하는 방법

---

### 목표:

**관측 자료와 모델 배경장을 조화롭게 섞어서 가장 그럴듯한 초기 상태를 만든다!**
...
3D-VAR의 목적은 다음의 **cost 함수** \(J(x)\)를 최소화하여 최적 상태를 추정하는 것입니다:

<p align="center">
  <img src="https://latex.codecogs.com/svg.image?J(x)=\frac{1}{2}(x-x_b)^TB^{-1}(x-x_b)+\frac{1}{2}(y-H(x))^TR^{-1}(y-H(x))" alt="3D-VAR Cost Function" />
</p>
...
- \(x\): 최적화할 상태 변수 벡터  
- \(x_b\): 배경 상태 (background, 예측값)  
- \(y\): 관측값 벡터  
- \(H(x)\): 관측 연산자 (모델 상태 → 관측 공간)  
- \(B\): 배경 오차 공분산 행렬  
- \(R\): 관측 오차 공분산 행렬  

- 첫 번째 항: **배경장과 분석 상태 간 거리** (background mismatch)  
- 두 번째 항: **관측값과 분석 상태 간 거리** (innovation)  

이 구조는 **베이즈 MAP 추정 (Maximum A Posteriori Estimation)**과 일치합니다.

---
## 최적화 문제

이제 \(J(x)\)를 **최소화**하는 \(x\)를 찾는 것이 목표입니다:

<p align="center">
  <img src="https://latex.codecogs.com/svg.image?x^*=\\arg\\min_xJ(x)" alt="Optimization Equation" />
</p>

즉, 배경 예측과 관측값을 모두 고려하여 **가장 그럴듯한 상태 \(x^*\)**를 추정


### 🧩 핵심 아이디어

* 모델 예보가 아무리 정교해도 **오차가 있음**
* 관측값도 현실이지만 **불완전하고 위치가 제한됨**
* 그래서 **‘최적의 타협점’을 수학적으로 계산**해내는 방식이 3DVAR
